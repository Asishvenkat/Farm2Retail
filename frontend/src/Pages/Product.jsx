...
const ContactSellerButton = styled.button`
  padding: 15px 30px; border: 2px solid teal; background-color: white;
  color: teal; cursor: pointer; font-size: 16px; font-weight: 500; border-radius: 8px;
  transition: all 0.3s ease;
  &:hover { background-color: teal; color: white; transform: translateY(-2px); }
  ${mobile({ flex: "1" })}
`;
  ...
  const handleContactSeller = async () => {
    ...
    try {
      ...
      const checkResponse = await axios.get(
        `${BASE_URL}messages/check-conversation/${currentUser._id}/${product.userId}`,
        {
          headers: {
            token: `Bearer ${token}`
          }

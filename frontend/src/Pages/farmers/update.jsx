    ...
    const fetchProduct = async () => {
      try {
        ...
        if (res.ok) {
          ...
          setBulkTiers(
            data.bulkTiers?.length > 0
              ? data.bulkTiers
              : [{ quantity: data.quantity || 10, price: data.price || '' }]
          );
        } else {
    ...
    try {
      ...
      const response = await fetch(`${BASE_URL}products/${id}`, {
        ...
        body: JSON.stringify({
          ...
          quantity: bulkTiers.reduce(
            (sum, tier) => sum + Number(tier.quantity),
            0
          ),
        }),
      });
  ...
  return (
    <>
      ...
      <div className="max-w-4xl mx-auto p-6 mt-10 bg-white shadow-md rounded-lg">
        ...
        {message && (
          <div
            className={`p-3 mb-4 rounded text-sm ${message.includes('✅') ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}
          >
            {message}
          </div>
        )}
            ...
            {bulkTiers.map((tier, index) => (
              <div key={index} className="flex items-center gap-4 mb-3">
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleBulkTierChange(index, 'price', Number(e.target.value))
                  }
                  className="p-2 border rounded w-40"
                  required
                />
              ...
              {product.images?.map((img, i) => (
                <div key={i} className="relative">
                  ...
                  <button
                    ...
                    type="button"
                    className="absolute top-0 right-0 bg-red-600 text-white rounded-full px-1 text-xs hover:bg-red-700"
                  >
                    ×
                  </button>
                </div>

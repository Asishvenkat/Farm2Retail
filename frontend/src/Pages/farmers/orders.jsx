...
import { Package, Calendar, User, Phone, MapPin, DollarSign, CheckCircle, Clock, XCircle } from 'lucide-react';
        ...
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          ...
          <div style={{ 
            ...
            {['all', 'pending', 'completed', 'cancelled'].map(status => {
              ...
              if (status === 'all') {
                ...
                count = orders.filter(o => o.status?.toLowerCase() === status).length;
              }
              ...
              return (
                <button
                  ...
                  style={{
                    padding: '10px 20px',
                    border: filter === status ? '2px solid #00796b' : '2px solid #ddd',
          ...
          {filteredOrders.length === 0 ? (
            <div style={{ 
              ...
              <h3 style={{ color: '#666', marginBottom: '10px' }}>No {filter !== 'all' ? filter : ''} orders found</h3>
              <p style={{ color: '#999' }}>Orders from retailers will appear here</p>
            </div>
          ) : (
              ...
              {filteredOrders.map((order) => (
                <div key={order._id} style={{ 
                  ...
                  <div style={{ 
                    ...
                    <h4 style={{ marginBottom: '10px', color: '#333', fontSize: '16px' }}>Customer Details</h4>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '10px' }}>
                      ...
                      {order.address && (
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: '#666', gridColumn: '1 / -1' }}>
                          <MapPin size={16} />
                          <span>
                  ...
                  <div style={{ marginBottom: '15px' }}>
                    <h4 style={{ marginBottom: '15px', color: '#333', fontSize: '16px' }}>Products Ordered</h4>
                    {order.products
                      .filter(item => item.productDetails && item.productDetails.farmerId === user._id)
                      .map((item, index) => (
                        <div key={index} style={{ 
                          display: 'flex', 
                          justifyContent: 'space-between', 
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: '15px', flex: 1 }}>
                            {item.productDetails.images && item.productDetails.images[0] && (
                              <img 
                            ...
                            <div>
                              ...
                              <div style={{ fontSize: '14px', color: '#666' }}>
                                Quantity: {item.quantity} {item.productDetails.unit}
                              </div>
                              <div style={{ fontSize: '13px', color: '#999', marginTop: '2px' }}>
                                Price per {item.productDetails.unit}: ₹{item.productDetails.price}
                              </div>
                            </div>
                          </div>
                          <div style={{ 
                  ...
                  <div style={{ 
                    ...
                    <span style={{ fontSize: '24px', fontWeight: '700', color: '#00796b' }}>
                      ₹{order.amount}
                    </span>
                  </div>
                </div>
